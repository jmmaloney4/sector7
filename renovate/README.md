...
- **`yaml-manifests.json`** - Tracks annotated dependency pins in YAML manifests that Renovate's built-in managers do not reliably cover in our repos. Supports two opt-in patterns: `# renovate: datasource=docker` above `image:` lines, and `# renovate: datasource=helm depName=<chart> registryUrl=<url>` above Helm chart `version:` lines. This is intended for Kubernetes manifests and Rancher `HelmChart` CRDs like the Garden RKE2 bootstrap files.
