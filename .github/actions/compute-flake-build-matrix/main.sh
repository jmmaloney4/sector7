...
"${lowprio[@]}" nix run github:nix-community/nix-eval-jobs/v2.35.1 "${nix_eval_args[@]}" -- --flake . --check-cache-status --meta --workers 1 --max-memory-size 3072 --select "(${select_expr}) \"${system}\"" >"$tmp_all"
